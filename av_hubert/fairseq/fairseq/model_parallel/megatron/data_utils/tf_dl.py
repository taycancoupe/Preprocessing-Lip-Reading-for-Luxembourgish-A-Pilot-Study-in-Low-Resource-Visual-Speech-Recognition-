...
class TFRecordDataLoader(object):
    def __init__(self, records, batch_size, max_seq_len, max_preds_per_seq, train, num_workers=2, seed=1, threaded_dl=False):
        ...
        self.record_converter = Record2Example({"input_ids": tf.FixedLenFeature([max_seq_len], tf.np.int64),
                                                "input_mask": tf.FixedLenFeature([max_seq_len], tf.np.int64),
                                                "segment_ids": tf.FixedLenFeature([max_seq_len], tf.np.int64),
                                                "masked_lm_positions": tf.FixedLenFeature([max_preds_per_seq], tf.np.int64),
                                                "masked_lm_ids": tf.FixedLenFeature([max_preds_per_seq], tf.np.int64),
                                                "masked_lm_weights": tf.FixedLenFeature([max_preds_per_seq], tf.float32),
                                                "next_sentence_labels": tf.FixedLenFeature([1], tf.np.int64)})
...
class Record2Example(object):
    ...
    def __call__(self, record):
        ...
        for k, v in list(example.items()):
            if v.dtype == tf.np.int64:
                example[k] = tf.to_np.int32(v)
        return example
