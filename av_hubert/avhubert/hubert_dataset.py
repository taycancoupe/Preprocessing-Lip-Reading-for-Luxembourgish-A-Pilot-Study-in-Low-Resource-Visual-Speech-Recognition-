...
class AVHubertDataset(FairseqDataset):
    ...
    def load_video(self, audio_name):
        path = os.path.join(self.audio_root, audio_name)

        if path.endswith(".npz"):
            data = np.load(path)
            feats = data["frames"] if "frames" in data else data["data"] if "data" in data else data["arr_0"]

            if feats.ndim == 4 and feats.shape[-1] == 3:
                feats = (0.299 * feats[..., 0] + 0.587 * feats[..., 1] + 0.114 * feats[..., 2]).astype(np.uint8)
            elif feats.ndim == 4 and feats.shape[-1] == 1:
                feats = feats[..., 0]
        else:
            feats = custom_utils.load_video(path)
